...
const CartPage: React.FC<CartPageProps> = ({ cart, addToCart, updateCartItem, updateCartItemSize, removeFromCart, clearCart, user, setUser, products }) => {
  ...
  return (
    <div className="bg-zinc-50 min-h-screen text-[#03401b] pt-10 pb-20">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <>
                  {selectedAddress ? (
                    <div className="relative">
                       ...
                       >
                         <div className="text-sm text-zinc-600">
                           <p className="text-white font-bold">{selectedAddress.receiverName} <span className="ml-2 text-[10px] bg-zinc-100 px-2 py-0.5 rounded text-zinc-500">{selectedAddress.isDefault ? 'DEFAULT' : 'HOME'}</span></p>
                       ...
                       {isAddressDropdownOpen && (
                         <div className="absolute top-full left-0 w-full bg-white border border-[#03401b]/10 rounded-xl mt-4 z-20 shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                           {sortedAddresses.map(addr => (
                             ...
                             >
                               <div className="flex-grow">
                                 <p className="font-bold text-[#03401b] text-sm">{addr.receiverName}</p>
                           ...
                           <button 
                            ...
                            className="w-full p-4 flex items-center justify-center space-x-2 text-[#03401b] hover:bg-[#03401b]/5 transition-colors"
                           >
                    ...
                    <button 
                      ...
                      className="w-full border-2 border-dashed border-[#03401b]/10 py-6 text-xs font-bold uppercase tracking-widest text-zinc-500 hover:border-[#03401b]/20 hover:text-[#03401b] transition-all"
                    >
                      + Add New Address
                    </button>
                  )}
            ...
            <div className="space-y-6">
                    ...
                    {sortedGifts.map((gift) => {
                        ...
                        return (
                            <div key={gift.id || gift._id} className={`snap-start min-w-[280px] border p-4 rounded-xl flex flex-col gap-3 ${isUnlocked ? 'bg-[#03401b]/5 border-[#03401b]/20' : 'bg-white border-[#03401b]/5'}`}>
                                ...
                                {gift.description && (
                                    <p className="text-[10px] text-zinc-600 font-medium italic border-t border-[#03401b]/5 pt-2">
                                        "{gift.description}"
                                    </p>
                                )}
                                ...
                                <button 
                                    ...
                                    className={`w-full py-2 text-[10px] font-black uppercase tracking-widest transition-colors rounded-lg flex items-center justify-center space-x-2 ${isClaimed ? 'bg-zinc-200 text-zinc-400 cursor-not-allowed' : isUnlocked ? 'bg-green-500 text-black hover:bg-white' : 'bg-zinc-100 text-zinc-600 cursor-not-allowed'}`}
                                >
                                    {isClaimed ? (
                                        <>
              ...
              {cartItems.length > 0 ? (
                  cartItems.map((item, idx) => {
                    ...
                    return (
                      <div key={`${item.productId}-${item.size}-${isThisItemFree}`} className={`flex flex-col md:flex-row gap-6 p-6 transition-all group relative ${isInvalidGift ? 'bg-red-500/5 border-red-500/20' : 'bg-white/30 border-[#03401b]/5 hover:border-[#03401b]/10'}`}>
                        {isThisItemFree && (
                            <div className={`absolute top-0 right-0 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-bl-xl z-10 ${isInvalidGift ? 'bg-red-500 text-[#03401b]' : 'bg-green-500 text-black'}`}>
                                {isInvalidGift ? 'Criteria Not Met' : 'Gift'}
                            </div>
                        )}
                        <div className="w-24 h-32 flex-shrink-0 overflow-hidden bg-zinc-100">
                        ...
                        <div className="flex-grow flex flex-col justify-between">
                          <div>
                            ...
                            <div className="flex justify-between items-start">
                              <Link to={`/product/${item.productId}`} className="text-lg font-black uppercase tracking-tight group-hover:text-[#03401b] transition-colors">{item.product.name}</Link>
                            ...
                            <div className="flex items-center space-x-4 mt-3">
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    {isThisItemFree ? (
                                       <span className="text-[10px] text-[#03401b] font-bold px-3 py-1.5 bg-zinc-100 rounded-lg border border-[#03401b]/10 uppercase tracking-widest">{item.quantity}</span>
                                    ) : (
                                       <CustomDropdown 
                                          value={item.quantity}
                                          label="Qty"
                          ...
                          <div className="flex items-center space-x-4 mt-4 md:mt-0">
                            {isThisItemFree ? (
                                <>
                                     <span className="text-xl font-black text-[#03401b]">
                                        ₹{giftDef?.price || 0}
                                     </span>
                                ...
                                <>
                                    <span className="text-xl font-black text-[#03401b]">₹{item.product.offerPrice}</span>
                ...
                <div className="py-20 flex flex-col items-center justify-center space-y-6 text-center border border-dashed border-[#03401b]/10 rounded-3xl bg-white/20">
                    <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center border border-[#03401b]/5">
                    ...
                    <Link to="/" className="mt-4 flex items-center justify-center space-x-2 bg-[#03401b] !text-white px-8 py-3 text-xs font-black uppercase tracking-widest hover:bg-zinc-800 transition-all rounded-lg mx-auto w-fit">
                        Explore Drops
                    </Link>
                </div>
              )}
          ...
          <div className="lg:col-span-4 space-y-6">
              ...
              {coupons.filter(c => c.isVisible).length > 0 ? (
                <div className="flex overflow-x-auto pb-4 gap-3 no-scrollbar">
                  {coupons.filter(c => c.isVisible).map(coupon => (
                    <div 
                      key={coupon.code}
                      className="ticket-border flex-shrink-0 px-8 py-5 flex flex-col items-center bg-zinc-50 border-[#03401b]/5 group"
                    >
                      <span className="text-xl font-black text-[#468057] tracking-tighter group-hover:scale-110 transition-transform">{coupon.code}</span>
                      <span className="text-[10px] text-black font-bold uppercase tracking-widest mt-1">
              ...
              <div className="space-y-3">
                <div className="relative">
                  <input 
                    ...
                    className="w-full bg-white border border-[#03401b]/10 px-4 py-4 text-sm font-bold uppercase tracking-widest focus:outline-none focus:border-pink transition-colors rounded-xl" 
                  />
                  <button 
                    onClick={() => handleApplyCoupon()}
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-[#03401b] text-xs font-black uppercase tracking-widest hover:text-[#03401b]"
                  >
                    Apply
                  </button>
                </div>
                ...
                {couponSuccess && isCouponValid && <p className="text-[10px] text-[#03401b] font-bold uppercase tracking-widest pl-2">{couponSuccess}</p>}
              </div>
            </div>

            {/* Price Breakdown */}
            <div className="bg-white p-8 rounded-3xl space-y-6">
              <h3 className="text-sm font-black uppercase tracking-widest border-b border-[#03401b]/5 pb-4">Order Summary</h3>
              <div className="space-y-4 text-sm tracking-wide">
                ...
                {discountAmount > 0 && (
                  <div className="flex justify-between text-zinc-600">
                    ...
                    <span className="text-[#03401b]">-₹{Math.round(discountAmount)}</span>
                  </div>
                )}
                {giftTotal > 0 && (
                   <div className="flex justify-between text-zinc-600">
                     <span>Gift Price Included</span>
                     <span className="text-[#03401b]">₹{giftTotal}</span>
                   </div>
                )}
                <div className="flex justify-between text-zinc-600">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-[#03401b] font-bold">0</span>
                  </div>
                </div>
                <div className="pt-4 border-t border-[#03401b]/5 flex justify-between items-center">
                  <span className="text-lg font-black uppercase tracking-tighter">Total Amount</span>
                  <span className="text-2xl font-black text-[#03401b]">₹{Math.round(total)}</span>
                </div>
              </div>
              ...
              <button 
                ...
                className={`w-full py-5 text-sm font-black uppercase tracking-[0.3em] transition-all flex items-center justify-center space-x-4 shadow-[0_10px_30px_rgba(255,255,255,0.05)] ${cartItems.length === 0 || !user || isCheckingOut || hasInvalidItems || !isCouponValid ? 'bg-zinc-200 text-zinc-400 cursor-not-allowed' : 'bg-white text-black hover:bg-green-500'}`}
              >
                <span>
            ...
            <button className="w-full text-center text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-[#03401b] transition-colors">
